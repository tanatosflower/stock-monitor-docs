...
stock-monitor の基本的な動作はわかっているが、各機能の中身をもう少し詳しく知りたい、という方を想定しています。まだ全体像をつかんでいない方は、先に[アプリ解説](./overview)をご覧ください。用語の定義は[用語集](./glossary)をご参照ください。
...
**テーゼ崩壊の監視は休日でも止まりません。** リリース発表や行政の通達は土日でも出ることがあり、保有テーゼを揺るがす材料は素早くキャッチして即時通知します。また、休日に拾った記事も上述の営業日ベースキャッシュに蓄積され、翌営業日の下落分析に引き継がれます。
...
| パターン | 意味 | 設計上の判定 | 現在の挙動 |
| --- | --- | --- | --- |
| **市場全体の下落** | 日経225と連動した連れ安。個別要因なし | ✗ | ✗ |
| **ファンダメンタル懸念** | 業績悪化・下方修正など個社に固有の悪材料あり | ✗ | ✗ |
| **資金ローテーション** | セクター/テーマ全体からの資金流出。個社の業績とは無関係 | ✓ | 保留（[詳細](./operations#current-status)） |
...
::: tip 「注目度不足」と「ノイズ」の見分け方
どちらも出来高が薄い状況で発生しますが、判定の根拠が正反対です。**ノイズ**は「悪材料が見当たらない」という消極的な事実にとどまるため、バーゲンだとは積極的に立証できず見送りになります。**注目度不足**は「買い手不在による需給の歪み」を LLM が積極的に立証したケースで、はじめて買い場候補に分類されます（LLM 判定専用）。「悪材料がない」＝「買い場」にはならない——これが[罠回避の原則](./overview#shiso-3)の境界線です。
:::
...
flowchart TD
    ...
    E -- はい、かつ連動範囲内 --> F[市場全体の下落<br/>場中に即時通知（下落パターン）]
    E -- いいえ --> G[第二層：LLM 判定]
    G --> H[7パターンに分類]
```
...
| **資金ローテーション保留** | [現在の暫定ステータス](./operations#current-status)に基づき、判定ログには残るが Discord 通知では「不明」として扱う |
| **情報真空ガード** | 価格アラートが出ているのに当日のニュースが1件も取得できていない（情報真空）場合は、買い場と立証する材料が欠けているため「不明（見送り）」に降格 |
...
引けサマリーは、その日の終値・前日比の一覧を1通にまとめて届けることに特化しています。「市場全体の下落」を含む全下落パターンは場中に即時通知（📋 下落パターン）されるため、引けサマリーには集約しません。
...
ここまでの機能がすべて組み合わさって、「5分ごとに東証銘柄を見張り、歪みを見つけて人間に差し出す」という動作が実現しています。それぞれの機能は独立して設計されていますが、下落パターン分析がニュース評価の結果を使い、テーゼ崩壊監視がそのニュースを出口判定にも活かすように、要所でつながっています。
