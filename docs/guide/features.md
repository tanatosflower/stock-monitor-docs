...
「資金ローテーション」は設計上は買い場候補のパターンですが、**現在は Discord への昇格通知を暫定的に保留中**です。最新の保留理由・解除条件は[運用・チューニングガイド](./operations#current-status)に集約しています。
:::

現在有効な買い場候補パターンは「注目度不足」「出尽くし」「過剰連れ安」の3つです。残りは見送りまたは[保留](./operations#current-status)です。
...
引けサマリーは、その日の「結果一覧」を1通で届けることに特化しています。「市場全体の下落」を含む全下落パターンは場中に即時通知（📋 下落パターン）されており、引けサマリーへの集約はおこないません。引けサマリー自体は、終値・前日比の一覧を1通にまとめて届ける役割を担っています。
