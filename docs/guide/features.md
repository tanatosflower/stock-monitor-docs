...
[アプリ解説](./overview.md) では「なぜこういう発想なのか」という思想と仕組みの全体像を説明しました。このページでは視点を変えて、**個々の機能が具体的に何をやっているか**を順番に見ていきます。「あの通知はどこから来るのか」「あの判定はどういう仕組みで動くのか」という疑問に答えるページです。
...
送信済みの記事 GUID をステートに蓄積し、一度通知した記事は再評価しません。ただし「通知 dedup（送信済みリング）」とは別に、**24時間のローリングキャッシュ**も持っています。こちらは下落パターン分析に文脈を供給するためのもので、通知済みかどうかとは独立して前後のサイクルに記事内容を渡し続けます。
...
| パターン | 意味 | 買い場? |
| --- | --- | --- |
| **市場全体の下落** | 日経225と連動した連れ安。個別要因なし | 通知集約 |
| **ファンダメンタル懸念** | 業績悪化・下方修正など個社に固有の悪材料あり | ✗ |
| **資金ローテーション** | セクター/テーマ全体からの資金流出。個社の業績とは無関係 | ✓（※現在は保留） |
...
「注目度不足」「出尽くし」「過剰連れ安」が買い場候補となるパターンです。「資金ローテーション」も設計上は買い場候補ですが、現在は観測蓄積フェーズのため Discord への昇格通知は保留中です。残りは見送りです。
...
flowchart TD
    ...
    B --> C{高重要度記事 AND 高出来高?}
    C -- はい<br/>全記事 negative/neutral --> D[ファンダメンタル懸念<br/>is_opportunity=False]
...
| **内生キル** | 原油が累積で下落しており、かつ石油業種銘柄の場合は「不明（見送り）」に降格（コモディティ価格連動の誤判定防止） |
| **資金ローテーション保留** | 「資金ローテーション（買い場）」は現在観測蓄積フェーズ。判定ログには残るが Discord 通知では「不明」として扱う |

判断できない・立証できない場合は、すべて「見送り」側に倒します（思想3：立証責任は買い場側に）。

### 通知スタイル

下落アラートが出た場合、`is_opportunity` の値や銘柄種別にかかわらず、必ず Discord に分析結果を送信します。スタイルは3種類です。

| スタイル | 条件 |
| --- | --- |
| 🎯 買い場候補 | 固定枠 + is_opportunity=True + 財務懸念なし |
| 🔎 要確認 | 固定枠で財務懸念あり、または流動枠で is_opportunity=True（堀が未検証） |
| 📋 下落パターン | is_opportunity=False（どの銘柄枠でも） |
...
## Discord 通知設計——チャンネルで分けて「必ず読む」を守る
