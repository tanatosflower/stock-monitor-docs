...
| **資金ローテーション** | セクター/テーマ全体からの資金流出。個社の業績とは無関係 | ✓（[保留中](./operations#current-status)） |
...
現在有効な買い場候補パターンは「注目度不足」「出尽くし」「過剰連れ安」の3つです。「資金ローテーション」は設計上は買い場候補ですが Discord 昇格は[暫定的に保留中](./operations#current-status)で、残りは見送りです。
