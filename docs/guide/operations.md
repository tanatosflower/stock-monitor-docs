...
相場の暴落パターンは多様で、LLM の判定挙動も事前にすべて読み切ることはできません。だからこそ[実戦データで締める](./overview#shiso-7)は完璧主義を捨てることを明言しています。**デフォルトは機会損失側（最も安全な側）に倒す**のを原則とし、あらゆる判定の軌跡を構造化ログに残した上で、**実戦データでロジックを後から締めていく**という方向性です。

これは[10年続ける持続性](./overview#shiso-6)の「素朴でも10年動き続ける」とも対になっています。一発で正解を当てようとするのではなく、観測し・分析し・微修正するサイクルを地道に回すことが、長く精度を保つための設計思想です。
...
「こういう粒度で全判定を記録している」ことが、チューニングを可能にする基盤です。ログがなければ「なんとなく精度が悪い」で終わってしまうところを、「**どの設定値（"ネジ"）が原因か**」まで追えるようになります。

::: tip ログはダッシュボードでも見られる
JSONL ログは `jq` での手作業のほかに、Streamlit 製のログ可視化ダッシュボード（概要／判定の内訳／流動枠スクリーニング／ニュースフィルタ／価格アラート／運用ヘルスの6タブ構成）でも閲覧できます。本体の処理とは分離した小さなアプリで、クラウド上にデプロイされています。後述の `download-blob-logs` / `check-azure-logs` が「集めて腰を据えて分析する」フローなら、ダッシュボードは「まずざっと眺める」入口です。
:::
...
手元に集めたログを `check-azure-logs` スキルで点検します。このスキルは動作面（例外・警告・発火頻度の欠測）と設計面（[実戦データで締める](./overview#shiso-7)の観点から「このネジを締めると精度が上がる」という示唆を導出）の両面で分析を行います。

::: tip check-azure-logs と実戦データで締める
このスキルの実行そのものが[実戦データで締める](./overview#shiso-7)の実践です。「どの閾値の過剰な厳しさが機会損失を生んでいるか」「どのプロンプトが誤判定を引き起こしているか」を、人間が読めるレポートとして出力します。報告するだけで、設定の変更はしません——最終判断は常に人間（[最終判断は人間](./overview#shiso-5)）です。
:::
...
これは[10年続ける持続性](./overview#shiso-6)の「低認知負荷・低コスト」を体現した設計です。設定変更のたびにデプロイパイプラインを走らせる手間がかからないため、細かいチューニングを気軽に試せます。
...
デフォルトは意図的に「締め側」に設定されています（[実戦データで締める](./overview#shiso-7)の「デフォルトを機会損失側に」）。
...
[10年続ける持続性](./overview#shiso-6)の「絞って必ず読む」を守るために、スコアは少し高めに設定しておくのが基本方針です。

---

## 見逃しの逆検証——誤検知と見逃しを両方つぶす

チューニングはつい「誤検知（罠を買い場と誤認）を減らす」方向にしか目が向きがちですが、同じくらい重要なのが**「見逃し（本当の買い場を素通り）を洗い出す」**逆方向の検証です。誤検知を減らしつつ見逃しも減らす——この両方をつぶすことが、長く精度を保つための要です。

[実戦データで締める](./overview#shiso-7)はこの考え方を「両肺呼吸」と呼んでいます。片肺（誤検知の排除）だけ鍛えると、システムが安全側に倒れすぎて機会損失が膨らみます。
...
この分析は「ネジを緩める方向性を示す」ための情報提供です。提示するだけで、パラメータを自動的に変えることはしません（[最終判断は人間](./overview#shiso-5)）。「確かにこのネジは締めすぎていた」と人間が判断してから、設定を手で変えます。
...
## 現在の暫定ステータス {#current-status}

実装済みのパターンのうち、現時点で **Discord 昇格が保留中** のものをここに集約します。保留は「設計上は有効だが、精度検証に必要な観測データが不足しているため、ライブ通知を一時的に抑制している」状態です。解除は [実戦データで締める](./overview#shiso-7) のサイクルで行われます。

| パターン | 状態 | 理由 |
| --- | --- | --- |
| **資金ローテーション** | Discord 昇格 保留中 | 観測蓄積フェーズ。十分なデータが蓄積し精度が確認できた段階で解除予定 |

**保留中のパターンの動作**:
判定ロジックは通常通り動作し、**判定ログには記録されます**。ただし Discord への通知では「不明（見送り）」として扱われます。チューニングのデータ源としては機能しているため、ログを参照することで蓄積状況を確認できます。

**解除のタイミング**: 十分な観測データが蓄積されて精度が確認できたと判断したら、コード側のガードを取り除く形で解除します。設定変更ではなくコード変更（PR）が必要です。最終判断は常に人間が行います（[最終判断は人間](./overview#shiso-5)）。
