...
役割を分け、それぞれに「できないこと」を課しているのがミソです。計画する人・書く人・チェックする人を分離することで、一人のエージェントが暴走して計画から逸れるのを防ぎます。
...
```mermaid
flowchart TD
    A[issue を選ぶ] --> B[Opus プランナーが計画を作成]
    B --> C{人間ゲート①<br/>計画を承認?}
    C -- 修正要望 --> B
...
レビュアーが「直しが必要（NEEDS_FIX）」と判定したら実装担当に差し戻し、それが2回続いたら計画自体を見直すために人間に相談する、というループも組み込まれています。AI に任せきりにせず、要所要所で人間が舵を握り続ける設計です。
