...
`main` への PR をきっかけに、GitHub Actions（`test.yml`）が **`lint` と `test` の2ジョブを並列**で実行します。

- **`lint` ジョブ** — `ruff check .`（Python 静的解析）＋ markdownlint（Markdown 体裁）を検証します。コードの品質と文書の整形を CI レベルで機械的に担保する役割です。
- **`test` ジョブ** — `test_unit.py` と `test_backtest.py` を順に実行します。流れは「チェックアウト → Python セットアップ → 依存インストール → テスト実行 → （失敗時のみ）Discord 通知」。
...
肝になるのは中核の3役です。役割を分け、それぞれに「できないこと」を課すことで、計画する人・書く人・チェックする人を分離し、一人のエージェントが暴走して計画から逸れるのを防ぎます。

### 2つの人間ゲート

この流れには、人間が必ず承認を挟む関所が**2か所**あります。これは[最終判断は人間](./overview#shiso-5)（最終判断は常に人間）を、開発プロセスにそのまま写し取ったものです。

なお、issue 番号を指定せずに呼んだときだけ、手前に「優先順位づけ」の工程が入ります（番号を最初から指定すれば飛ばします）。ここで人間が番号を選ぶ操作は、着手対象を決める整理であって、承認ゲートには数えません。
...
agent-resolve では**複数の issue を worktree 並走で同時着手**できます。それぞれの issue が独立した作業ブランチ（worktree）を持ち、実装・テストを並列で進められます。

衝突の判定単位は「同じファイルの同じ箇所（ハンク）を触るかどうか」です。非重複ハンクは git の自動マージに任せれば整合します。「`docs/` 配下を何か触れば全部衝突」とはみなしません（そう解釈すると実質すべてが直列化してしまうため）。

```mermaid
flowchart LR
    A[issue A<br/>worktree-A] --> MA[PR-A を先に<br/>main へ squash merge]
    B[issue B<br/>worktree-B] --> MB[PR-B は main マージ後<br/>git merge origin/main → CI → merge]
    MA --> MB
```

**マージの規律**は以下のとおりです。後発 PR は先行 PR が main にマージされてから `git merge origin/main` で最新を取り込み、競合を解消してから CI を再度通過させてマージします。`git rebase` は使いません（squash merge で履歴を捨てるため、rebase の利得がなく force-push が必要になるだけです）。

競合解消で触った差分がドキュメント・ログスキーマ・不変条件に関わる場合は、`issue-reviewer` に再判定を依頼します。

なお、プランナーとレビュアーはどちらも Opus 枠を消費します。並列数を増やすほど枠が先に詰まりやすくなる点は念頭に置いておきましょう。

## プランレビュー（標準モード）

agent-resolve とは別に、通常のプランモードで立てた計画も、ユーザー承認を求める前に必ず専用の Opus レビュアー（`plan-reviewer`）にレビューさせる、という無条件のゲートがあります。プランを立てた以上は軽微・trivial でも例外にしません。観点は、不変条件・思想への適合、要件の過不足、ドキュメントの責務分担、テスト方針、既存資産の再利用機会、副作用リスクなど。

なお「単一ファイルかつ仕様・思想・不変条件・ドキュメント仕様に触れない小変更」は、そもそもプランを立てずに済ませる **軽量パス**があります。軽量パスを超えたら issue 化せずにこの標準プランモードで扱い、「並列着手・大規模・厳密なレビューが必要」という場合だけ agent-resolve（issue 化）に乗せます。
