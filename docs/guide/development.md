---
title: 開発プロセス
description: stock-monitor がどう開発・テスト・リリースされているか。トランクベース運用、テストの責任分界、CI/CD、マルチエージェント開発フローを、はじめての人向けに解説します。
---
...
これは思想6（個人開発としての持続可能性）に基づく意図的な選択です。個人開発では、緊急時に人間の判断で押し通せる自由を残しておくほうが、硬すぎるルールに縛られて止まるより健全だ、という割り切りです。代わりに「テスト合否はちゃんと見える」「失敗すれば Discord に飛ぶ」という形で、強制ではなく**可視化**で品質を担保します。
...
この流れには、人間が必ず承認を挟む関所が**2か所**あります。これは思想5（最終判断は常に人間）を、開発プロセスにそのまま写し取ったものです。
